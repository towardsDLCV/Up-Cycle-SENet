...
def pesq_loss(clean, noisy, sr=16000):
    try:
        pesq_score = pesq(fs=16000, ref=clean, deg=noisy, mode='wb')
    except:
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y):
        ...
        mag_x = torch.sqrt(x[:, :257] ** 2 + x[:, 257:] ** 2).unsqueeze(1)
        mag_y = torch.sqrt(y[:, :257] ** 2 + y[:, 257:] ** 2).unsqueeze(1)
        xy = torch.cat([mag_x, mag_y], dim=1)
